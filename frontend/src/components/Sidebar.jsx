  ...
  return (
    <aside className='w-64 bg-base-200 border-r border-base-300 hidden md:flex flex-col h-screen sticky top-0'>
      ...
      <nav className='flex-1 p-4 space-y-1'>
        <Link to="/"
        className={`btn btn-ghost rounded-2xl justify-start w-full gap-3 px-3 normal-case ${currentPath ==="/" ? "btn-active" :""}`}>
            <HomeIcon className='size-5 text-base-content opacity-70'/>
            <span>Home</span>
        </Link>

        <Link to="/friends"
        className={`btn btn-ghost rounded-2xl justify-start w-full gap-3 px-3 normal-case ${currentPath ==="/friends" ? "btn-active" :""}`}>
            <UsersIcon className='size-5 text-base-content opacity-70'/>
            <span>friends</span>
        </Link>

        <Link to="/notification"
        className={`btn btn-ghost rounded-2xl justify-start w-full gap-3 px-3 normal-case ${currentPath ==="/notification" ? "btn-active" :""}`}>
